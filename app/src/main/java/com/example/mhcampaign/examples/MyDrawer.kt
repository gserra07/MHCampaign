...
@Composable
fun MyDrawer(scope: CoroutineScope, items: List<MenuItem>, selectedItem: MutableState<MenuItem>, drawerState: DrawerState, content: @Composable () -> Unit) {
...
fun MyDrawerPreview() {
    MHCampaignTheme(darkTheme = false, dynamicColor = false) {
        ...
        MyDrawer(scope = scope, items = menuItems, selectedItem = selectedItem, drawerState = drawerState, content = {
            when (selectedItem.value.index) {
                ...
                1 -> {
                    MyScaffold(title = selectedItem.value.name,
                            ...
                            content = { HunterViewHolderPreview() })
                }
                else -> {
                    MyScaffold(title = selectedItem.value.name,
                            scope = scope,
